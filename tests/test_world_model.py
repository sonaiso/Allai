...
def _build_full_state(text: str) -> ProofState:
    """Run the pipeline up to and including proposition_closure."""
...
class WorldModelExtractionTests(unittest.TestCase):
    def test_world_model_extraction_from_composition(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)

        self.assertIn("entities", state.world_model)
        self.assertIn("events", state.world_model)
        self.assertIn("relations", state.world_model)
        self.assertIn("laws", state.world_model)
        self.assertIn("causality", state.world_model)
        self.assertIn("goals", state.world_model)
        self.assertIn("judgments", state.world_model)
        self.assertIn("uncertainty", state.world_model)
        self.assertIn("blockers", state.world_model)
        self.assertGreater(len(state.world_model["entities"]), 0)
        self.assertGreater(len(state.world_model["laws"]), 0)

    def test_world_model_trace_event_emitted(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)
    ...
    def test_world_model_laws_include_linguistic_defaults(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)

        law_types = [law.get("law_type") for law in state.world_model["laws"]]
        self.assertIn("linguistic", law_types)


class WorldModelClosureTests(unittest.TestCase):
    def test_world_model_closure_requires_entity_and_law(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)
    ...
    def test_world_model_closure_trace_event_emitted(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)
        apply_world_model_closure(state)

        trace_events = [t["event"] for t in state.trace_chain]
        self.assertIn("world_model_closure", trace_events)

    def test_world_model_closure_blocked_when_no_entities(self) -> None:
        ...
        self.assertIn("no_entities_extracted", state.world_model["blockers"])

    def test_world_model_closure_blocked_when_no_laws(self) -> None:
        state = ProofState(ingress_text="x")
        state.proposition_closed = True
        state.world_model = {
        ...
        self.assertFalse(state.world_model_closed)
        self.assertIn("no_laws_available", state.world_model["blockers"])

    def test_world_model_closure_blocked_when_proposition_not_closed(self) -> None:
        state = ProofState(ingress_text="x")
        ...
        self.assertFalse(state.world_model_closed)
        self.assertIn("proposition_not_closed", state.world_model["blockers"])


class WorldModelUncertaintyTests(unittest.TestCase):
    def test_world_model_uncertainty_logged_when_causality_empty(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)

        uncertainty = state.world_model.get("uncertainty", [])
        self.assertIn("causality_not_detected", uncertainty)

    def test_world_model_uncertainty_logged_when_goals_empty(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)

        uncertainty = state.world_model.get("uncertainty", [])
        self.assertIn("goals_not_detected", uncertainty)


class WorldModelJudgementGateTests(unittest.TestCase):
    def test_judgement_rejected_without_world_model_closed(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)
        # Deliberately skip apply_world_model_closure so world_model_closed = False
        with self.assertRaises(GateViolationError):
            transition_proposition_to_judgement(state)

    def test_e2e_includes_world_model(self) -> None:
        state = _build_full_state("النص / واضح")
        extract_world_model(state)
        apply_world_model_closure(state)
        transition_proposition_to_judgement(state)
